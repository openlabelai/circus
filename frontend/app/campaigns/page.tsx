...
export default function CampaignsPage() {
  ...
  const { setActiveCampaign, refreshCampaigns: refreshCtx } = useCampaign();
  const [campaigns, setCampaigns] = useState<Campaign[]>([]);
  const [creating, setCreating] = useState(false);
  const [newName, setNewName] = useState("");

  const load = () => {
    getCampaigns().then((d) => setCampaigns(d.results || [])).catch(console.error);
  };
  ...
  const handleCreate = async () => {
    ...
    await createCampaign({ name: newName.trim(), color });
    setNewName("");
    setCreating(false);
    load();
    refreshCtx();
