...
export default function FansPage() {
  ...
  const load = useCallback(async () => {
    if (!id) return;
    try {
      const [agentData, campaignData] = await Promise.all([
        getAgents(id),
        getCampaign(id),
      ]);
  ...
  const target = campaign?.target_persona_count || 0;
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-gray-500">
          ...
          <p className="text-sm">
            Click &quot;Generate Fans&quot; to create synthetic fan personas for this campaign
          </p>
        </div>
      ) : (
